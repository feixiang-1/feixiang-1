...
const dnsConfig = {
  ...
  "proxy-server-nameserver": ["114.114.114.114", "119.29.29.29"],
...
function main(config) {
  ...
  config["proxy-groups"] = [
    ...
    },
    {
      ...
      "type": "select",
      "include-all": true,
      "filter": "(?i)台湾|tw|美国|usa|加拿大|ca|新加坡|sg|日本|jp|chatgpt|英国|uk|德国|de|越南|vn|巴西|br|阿根廷|ar",
    ...
    },
    {
      ...
      "type": "select",
      "include-all": true,
      "filter": "(?i)台湾|tw|美国|usa|加拿大|ca|新加坡|sg|英国|uk|德国|de|越南|vn|巴西|br|阿根廷|ar",
  ...
  config["proxies"] = config["proxies"]?.map(proxie =>

    proxie.type === 'hysteria2'
      ? { ...proxie, name: proxie.name.concat(` 节点数:${config["proxies"].length} (fixed)`), up: "120 Mbps", down: "600 Mbps" }
      : { ...proxie, name: proxie.name.concat(` 节点数:${config["proxies"].length}`) }
  );
